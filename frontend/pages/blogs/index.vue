<template>
    <div>
        <MainBanner 
            ...
            alt="What is happening at mymonty blogs"
            page="Blogs"
        />
                    ...
                    <div class="flex flex-col gap-8">
                        <div v-for="number in perPage" class="grid lg:grid-cols-2 justify-start items-center max-h-96 bg-[#ECECEE] rounded-[30px]">             
                            <div class="flex flex-col justify-start items-start gap-8 py-6 px-8 lg:p-12 xl:p-24">
                                <div class="rounded-md w-1/3 h-[25px] bg-[#f2f2f2]"></div>
                                <div class="rounded-md w-1/2 h-[30px] bg-[#f2f2f2]"></div>
                            ...
                            <div class="w-full h-full rounded-b-[30px] lg:rounded-r-[30px] lg:rounded-l-none overflow-hidden">
                                <div class="rounded-md h-full w-full bg-[#f2f2f2]"></div>
                            </div>
                        </div>
                    </div>
                </div>
...
<script setup>
    ...
    const { data: posts, pending, error, refresh } = await useFetch('http://backend.mymontyteuriope.localhost/wp-json/wp/v2/posts', {
    ...
    const loadMorePosts = async () => {
        if (currentPage.value < totalPages.value) {
            ...
            const newPosts = await $fetch('http://backend.mymontyteuriope.localhost/wp-json/wp/v2/posts', {
                query: { categories: 3, per_page: perPage.value, page: currentPage.value, _embed: '1' }
            });
